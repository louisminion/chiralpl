module spectra
    ...
    implicit none
    private
    public :: absorption, pl, calc_vibpl_spec_per_config, write_pl, calc_abs_spec, write_abs, cd, calc_cd_spec, write_cd,cpl,calc_cpl_spec,write_cpl
    contains
...
!
        ...
        subroutine cpl(rot_strengths,H,lattice_dimx,lattice_dimy,lattice_dimz,lattice_index_arr,one_particle_index_arr,two_particle_index_arr,general_counter,mu_xyz,max_vibs,fc_ground_to_neutral,k,mu_0,x_spacing,y_spacing,z_spacing,r_xyz)
            !LM This routine needs to be reworked. It might be easier to just loop over all states that emit 
            !LM rather than separating into vibronic contributions to avoid the headache that is figuring out all the possible
            !LM terminal states with v vibrations etc
            
            implicit none
            real(wp), intent(inout) :: rot_strengths(:)
                                ...
                                do i_z2=1,lattice_dimz
                                    ...
                                    do vib=0,max_vibs
                                        do vib2=0,max_vibs
                                            h_i = one_particle_index_arr( n, vib )
                                            c_nv = H(h_i,1)
                                            h_i = two_particle_index_arr( m, vib2 ,n,2)
                                            if (h_i .eq. empty) cycle
                                            c_nvmv = H(h_i,1)
                                            R_02 = R_02 + fc_ground_to_neutral(vib,2)*fc_ground_to_neutral(0,vib2)*c_nv*c_nvmv*dot_product(crossproduct,rdiff)
                                        end do
                                    end do
                                    ...
                                    do i_x3=1,lattice_dimx
                                        do i_y3=1,lattice_dimy
                                            do i_z3=1,lattice_dimz
                                                ...
                                                do vib2=0,max_vibs
                                                    do vib3=0,max_vibs
                                                        h_i = two_particle_index_arr(n1,vib2,n,2)
                                                        if (h_i .eq. empty) cycle
                                                        c_nv = H(h_i,1)
                                                        h_i = two_particle_index_arr(n2,vib3,n,2)
                                                        if (h_i .eq. empty) cycle
                                                        c_nvmv = H(h_i,1)
                                                        R_02 = R_02 + fc_ground_to_neutral(vib2,0)*fc_ground_to_neutral(vib3,0)*c_nv*c_nvmv*dot_product(crossproduct,rdiff)
                                                    end do
                                                end do
                                            end do
                                        end do
        ...
        subroutine calc_cpl_spec(vt,spec_steps,w00,lw, EVAL,general_counter, temp,kB,pi,rot_strengths,cpl_spec)
            implicit none
            integer(wp), intent(in) :: spec_steps
            integer, intent(in) :: general_counter
            integer(wp) :: spec_point, j, vt
            real(wp) :: spectrum_start, spectrum_end, energy
            real(wp) :: exciton_energy, boltzfactor, boltzsum
            ! real(wp), allocatable :: occupation_factors(:)
            real(wp), intent(in) :: w00, lw, temp, kB, pi
            ...
            real(wp), intent(out) :: cpl_spec(:)
            real(wp), intent(in) :: rot_strengths(:)
        ...
        subroutine write_cpl(cpl_spec,w00,hw,max_vibs,spec_steps,lw,INPUT_NAME)
            implicit none
            character*256 :: eval_out_f
            integer(wp) :: spec_point
            real(wp) :: step, energy,spectrum_start,spectrum_end
            integer(wp), intent(in) :: spec_steps, max_vibs
            character*256, intent(in) :: INPUT_NAME
            real(wp), intent(in) :: w00,hw,lw
            real(wp), intent(in) :: cpl_spec(:)
            spectrum_start = 0.0_wp
            spectrum_end = w00+20.0_wp*lw
            ...
            111 format(*(A14, :, ","))
            write(671,*) trim(INPUT_NAME)
            write(671,111) 'Energy','CPL'
            do spec_point=1,spec_steps
                energy = spectrum_start+ ((spectrum_end-spectrum_start)*(1.0_wp*spec_point))/(spec_steps*1.0_wp)
                write( 671, 110 ) energy*hw, cpl_spec(spec_point)
            end do
            close(671)
        end subroutine
