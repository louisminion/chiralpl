...
!****************************************************************************************************        
  ...
  !*******************************************************************************************************************************!
    !We build the hamiltonian here; the one particle hamiltonian, two particle hamiltonian and the one particle -  two particle hamiltonian
        ...
        if ( one_state ) write(*,*) 'FORMING 1 PARTICLE HAMILTONIAN'
        if ( two_state ) call hamiltonian_2p(config)
        if ( two_state ) then
         write(*,*) 'FORMING 2 PARTICLE HAMILTONIAN'
        end if
        if ( one_state .and. two_state ) call hamiltonian_1p2p(config)
        if ( one_state .and. two_state ) write(*,*) 'FORMING 1 PARTICLE - 2 PARTICLE HAMILTONIAN'
 !*******************************************************************************************************************************!        
...
subroutine hamiltonian_1p(config)
    ...
    do vib1 = 0, vibmax
        ...
        do vib2 = 0, vibmax
            ...
            !it is hermitian                
            h( hx2, hx1 ) = h( hx1, hx2 )
            write(7,*) 'h(',hx1,hx2,')', h(hx1,hx2)
            write(7,*) 'h(',hx2,hx1,')', h(hx2,hx1)
        end do
        end do
        end do
        end do
