...
program chiralpl
    ...
    allocate(xpl_osc_configavg(max_vibs+1,general_counter))
    allocate(ypl_osc_configavg(max_vibs+1,general_counter))
    ...
    !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(o) SHARED(abs_osc_strengths_x_configavg,abs_osc_strengths_y_configavg,xpl_osc_configavg,ypl_osc_configavg,general_counter,bool_one_particle_states, A_covar)
    !$OMP DO
    do o = 1, configs
        EVAL = 0.0_wp
        H = 0.0_wp
    ...
    ! Save eigenvalue logic (only if sigma 0)
        ...
        !     end if
        ! end if
        write(*,*) 'Running thread', omp_get_thread_num()
        ...
        ! write(*,*) xpl_osc(:,1)
        ! write(*,*) ypl_osc(:,1)
        ! write(*,*) SIZE(xpl_osc,dim=1),SIZE(xpl_osc,dim=2)
        call absorption()
        !$OMP CRITICAL
...
subroutine test()
    open(unit=57,file='t.csv')
    write(57,*) 'THREAD REACHED TEST'
    close(unit=57)
end subroutine
